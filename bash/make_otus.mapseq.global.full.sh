...
export fileDB=$folderRefDB/full-ssu-db3.unaligned.fasta;
export fileDBclust=$folderRefDB/full-ssu-db3.mscluster;
#Taxonomies
export fileTaxonomy=$folderRefDB/full-ssu-db3.tax_LTP119.ms;
export fileOTUtaxAL=$folderRefDB/full-ssu-db3.al.otutax;
export fileOTUtaxCL=$folderRefDB/full-ssu-db3.cl.otutax;
export fileOTUtaxSL=$folderRefDB/full-ssu-db3.sl.otutax;
export fileOTUtaxUC=$folderRefDB/full-ssu-db3.otu_tax.uclust.R.otutax;
################################################################################
...
for v in v13 v35; do
  ...
  export fileMSout=$folderData/$v.nonchimeric.filtered.full_db.ms;
  ...
  #Run mapseq
  $MAPSEQ --nthreads 40 -cfile $fileDBclust $fileUnaligned $fileDB $fileTaxonomy $fileOTUtaxAL $fileOTUtaxCL $fileOTUtaxSL $fileOTUtaxUC > $fileMSout;
  ...
  for cm in al cl sl; do
    ...
    #Make OTUs
    $HPCCLUST -makeotus $fileAlignmentDenoised $fileOutClust.$cm 0.97 | grep -v "^#" | perl -pi -e 's/^>/\n>/g' | sed "1d" > $fileTMP;
  ...
  perl -e '
    ...
    ##############################
    #Read closed reference OTU file
    ...
    while ($line = <OTU>) {
      ...
      if ($line =~ /^>/) {
        ...
        while ($acc = <OTU>) {
           chomp $acc;
           if ($acc =~ /^$/) {push @otu_order => $otu_name; next LINE}
           $raw_OTU{$otu_name}{$acc} = 1;
        }
      }
    }
    close OTU;
